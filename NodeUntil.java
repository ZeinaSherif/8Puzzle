...
public class NodeUntil {
    ...
    public static void printSolution(Node goalNode, Set<String> visitedNodes, Node root, int time) {
        ...
        totalCost=solutionStack.size()-1;
        String sourceState = root.getState();
        String destinationState;
        
        String []transition=new String [solutionStack.size() - 1];
        for (int i = solutionStack.size() - 1; i >= 0; i--) {
            ...
            if (!sourceState.equals(destinationState)) {
                System.out.println("Move " + destinationState.charAt(sourceState.indexOf('0')) + " " + findTransition(sourceState, destinationState));
                 ...
                 transition[i]=findTransition(sourceState, destinationState).toString();
                         }
        ...
        System.out.println("____________________________________________________________________");
        System.out.println("-> Path To Goal:  " + Arrays.toString(transition));
        System.out.println("-> Number of nodes expanded:  " + (visitedNodes.size()));
        System.out.println("-> Cost of path: " + totalCost);
        System.out.println("-> Search depth: " + time);
